...
public class CompanyController {
	...
	public ResponseEntity<?> getAllCompanyDetails() {
		...
		if (companyList != null) {
			...
			return ResponseEntity.ok().cacheControl(cacheControl).body(
					ResponseHandler.generateResponse("Succesfully retrieved the data", HttpStatus.OK, companyList));
		} else {
			return new ResponseEntity<String>("No such company present", HttpStatus.NO_CONTENT);
	...
	public void getJWTToken(@RequestBody User user) {
    	...
    	List<ServiceInstance> instances = discoveryClient.getInstances("usermanagementservice");    	
